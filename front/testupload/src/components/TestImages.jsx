...
const TestImages = () => {
  ...
  return (
    ...
    <div>
      ...
      <div><Link className='link' to="/">Retour</Link></div>
      ...
      <div><Link className='link' to="/view">Voir vos images</Link></div>
    </div>
    ...
    <img src={`data:image/jpg;base64,${data}`} alt=""/>
      {/* <div className='images-sort' >
        ...
        data.map((image,i)=> <img className='images images-sort' key={i} src={`https:localhost:8000/images/${image.img_nom}`} alt={image.img_nom} />)}
      </div> */}
      <button onClick={()=>onClicked()} >voir image</button>
    </div>
  </div>
  );
};
